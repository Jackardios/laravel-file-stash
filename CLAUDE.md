...
**Supported versions:** PHP ^8.2, Laravel ^11 || ^12 || ^13
...
1. **Read path** (`tryReadExisting`): `fopen('rb')` → `LOCK_SH` → `fstat`: `nlink == 0` → retry (entry was deleted/re-published); `size == 0` **and** `legacy_lifecycle_lock=true` → delete (in-place SH→EX upgrade + verify guard) + retry; with legacy off, zero-byte entries are valid and served. Hot reads never touch the claim.
...
3. **Deletes**: core is `unlinkLocked(path, stream?, verify?)` — `LOCK_EX|LOCK_NB` (fresh fd or in-place upgrade of a passed stream, whose ownership it takes) + compare `fstat($fd)` vs `stat($path)` dev/ino — never delete an entry that was concurrently re-published over the same path. `deleteEntry()` adds eviction metrics + `CacheFileEvicted`; temp/claim GC calls `unlinkLocked` directly (no events).
...
`batch()`/`batchOnce()` hold it shared; `forget()`, once-cleanup, `prune()` (shared), and `clear()` (exclusive) coordinate through it. While `legacy_lifecycle_lock => true` (default, for v4/v5 rolling deploys) the v4-style lock in the system temp dir is taken as well, always in the order legacy → new. Chunked batches (`count > batch_chunk_size`) release per-file SH locks before the callback — only the lifecycle lock protects the callback window then.
